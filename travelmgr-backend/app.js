/**
 * Travel Manager Express API entry point.
 *
 * Middleware order matters:
 * 1. CORS (credentials enabled for cross-origin frontend)
 * 2. JSON body parser
 * 3. PostgreSQL session store (cookie-based auth for Vercel → Render)
 * 4. JWT extraction (`tokenExtractor`) so routes can read `req.user`
 *
 * Production cookies use `sameSite: 'none'` + `secure: true` for cross-site sessions.
 * Core CRUD routes are mounted before AI and admin routes.
 */
